    ...
    <table style="width: 100%;position: relative;top: 120px;" v-show="!store.state.enable_big_icon_mode">
      <tr>
        <td v-show="store.state.sort_state.includes('desc')" style="width:79.5%;font-size: 7px;color: #727272;" class="attribute-td" v-on:click="set_file_name_order_asc()" >
          ...
          <svg fill="currentColor"  style="color:#575757;width: 15px;height: 15px;margin-bottom: -4px" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512"><!--! Font Awesome Pro 6.1.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. --><path d="M311.9 335.1l-132.4 136.8C174.1 477.3 167.1 480 160 480c-7.055 0-14.12-2.702-19.47-8.109l-132.4-136.8C-9.229 317.8 3.055 288 27.66 288h264.7C316.9 288 329.2 317.8 311.9 335.1z"/></svg>
        </td>
        <td v-show="store.state.sort_state.includes('asc')" style="width:79.5%;font-size: 7px;color: #727272;" class="attribute-td" v-on:click="set_file_name_order_desc()" >
          <span style="margin-left: 34px"> 名称 </span>
          <svg fill="currentColor" style="color:#575757;width: 15px;height: 15px;margin-bottom: -4px" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512"><!--! Font Awesome Pro 6.1.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. --><path d="M27.66 224h264.7c24.6 0 36.89-29.78 19.54-47.12l-132.3-136.8c-5.406-5.406-12.47-8.107-19.53-8.107c-7.055 0-14.09 2.701-19.45 8.107L8.119 176.9C-9.229 194.2 3.055 224 27.66 224z"/></svg>
        </td>
...
file.suffix!=='doc'&&file.suffix!=='docx'&&file.suffix!=='ppt'&&file.suffix!=='pptx'&&file.suffix!=='pdf'&&file.suffix!=='exe'">
      ...
      <!--↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓ 列表显示区域  -->
      <table class="fileTable" v-show="!store.state.enable_big_icon_mode" style="width:calc(100% - 20px);border-collapse: collapse;">
        <!--↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓  文件夹显示区域-->
        ...
        <tr class="table_area" v-for="file in file_list.file_data" :key="file">
          ...
          <td class="file_infor_td" v-on:click="open_video_or_image(file)"  v-on:click.right="click_drop_down(file)" style="font-size: 14px;width: 75%;color: #c0c0c0;table-layout: fixed" >
            <div style="display: inline"  v-lazy-container="{ selector: 'img', loading: loading_image}">
              <img class="file_icon" style="border-radius: 5px;width: 30px;margin-right: 15px" :data-src="show_thumbnail(file)" v-show="file.suffix=='jpg'||file.suffix=='png'||file.suffix=='jpeg'||file.suffix=='bmp'||file.suffix=='gif'">
            </div>
      ...
      <el-dropdown trigger="click" id="my_drop_down" placement="bottom-start">
        ...
        <template #dropdown>
          <el-dropdown-menu>
            ...
            <el-dropdown-item v-on:click="open_share_file_dialog(drop_down_file_infor)" style="color: #e0e0e0;font-size: 15px;width: 150px;margin-left: 8px;margin-bottom: 5px">
            ...
            <el-dropdown-item style="color: #e0e0e0;font-size: 15px;width: 150px;margin-left: 8px;margin-bottom: 5px" @click="edit_name(drop_down_file_infor)">
...
function move_file_to_recycle_bin(file_path)
{
  axios.get("/move_file_to_recycle_bin", {
    ...
    if (response.data===true)
    {
      ...
      axios.get("/inToFolder_searchFile", {
        params:{
          folder_path:route.params.path
        }
...
{
  const x = checked_files_group.value;
  axios.get("/crumbs_path", {
    params:{
      folder_path:x[0],
      user_name:cookie.get("user_name"),
      multi_files_state:true
    }
  }).then(function (response) {
    crumbs_path_in_move_file_dialog.crumbs_data=response.data;
    if(response.data===null)
...
watch(()=>route.path,()=>
{
  store.commit("set_current_path_for_create_folder_function",route.params.path);

  store.commit('set_current_page','home');

  //设置目标路径，表示当用户在此路径的文件页面上传某个文件时，文件最后保存的路径为此设置的目标路径
  store.commit("set_current_upload_path",route.params.path);//从路由的参数中取值，参数保存的是当前路径的绝对路径
  ...
  axios.get("/crumbs_path", {
    ...
    store.commit('set_crumbs_path',response.data);
    // console.log(store.state.crumbs_path);
  })
...
onBeforeMount(()=>
{
  store.commit("set_current_path_for_create_folder_function",route.params.path);
  store.commit('set_current_page','home');
  store.commit("set_current_upload_path",route.params.path);
...
watch(()=>store.state.one_file_has_been_uploaded,()=>
{
  ...
  if (route.params.path===store.state.check_current_path_and_upload_path) //判断当前用户处在的路径是否是当前上传成功的文件的存储的目标路径，是则刷新文件列表
  {
    axios.get("/inToFolder_searchFolder", {
      params:{
        //router带参数
